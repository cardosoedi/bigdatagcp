from datetime import datetime


def _date_fields(stock, field):
    date = datetime.strptime(stock[field], '%d/%m/%Y')
    stock[field] = date.strftime('%Y-%m-%d %H:%M:%S')
    return stock


def _number_fields(stock, field_prefix):
    result = dict()
    for k, v in stock.items():
        if isinstance(v, str) and k.startswith(field_prefix):
            if v.find('%') >= 0:
                result[k + '%'] = float(v.replace('%', '').replace(',', '.'))
            else:
                result[k] = float(v.replace(',', '.'))
        else:
            result[k] = v
    return result


def _integer_fields(stock, field_prefix):
    result = dict()
    for k, v in stock.items():
        if isinstance(v, str) and k.startswith(field_prefix):
            result[k] = int(v.replace(',', '.'))
        else:
            result[k] = v
    return result


def treat_all_fields(stock):
    num_fields = ['cotacao',
                  'min_52_sem',
                  'max_52_sem',
                  'nro_acoes',
                  'vol_med_2m',
                  'valor',
                  'result_int',
                  'rec_servicos',
                  'ebit_ultimos',
                  'receita_liquida',
                  'lucro_liquido',
                  'indicadores_fundamentalistas',
                  'oscilacoes',
                  'balanco_patrimonial']

    int_fields = ['nro_acoes',
                  'vol_med_2m',
                  'valor',
                  'result_int',
                  'rec_servicos',
                  'ebit_ultimos',
                  'receita_liquida',
                  'lucro_liquido',
                  'balanco_patrimonial']

    stock = _date_fields(stock, 'ult_balanco_processado')
    stock = _date_fields(stock, 'data_ult_cot')
    for field in int_fields:
        stock = _integer_fields(stock, field)
    for field in num_fields:
        stock = _number_fields(stock, field)
    return stock
